...
# ### Train Test and Feature Conversion
...
#model_path = "D:/model/GBT_model3030"
...
# Evaluate the model
evaluator = RegressionEvaluator(labelCol="price", predictionCol="prediction", metricName="r2")
r2_gb_t= evaluator.evaluate(predictions)
mse_gb_t= evaluator.evaluate(predictions, {evaluator.metricName: "mse"})
mae_gb_t= evaluator.evaluate(predictions, {evaluator.metricName: "mae"})
rmse_gb_t= evaluator.evaluate(predictions, {evaluator.metricName: "rmse"})
print("R2 on test data = %g" % r2_gb_t)
print("Mean squared error:", mse_gb_t)
print("Mean absolute error:", mae_gb_t)
print("Root mean squared error:", rmse_gb_t)
# Show predictions
predictions.select("final_feature_vector", "price", "prediction").show()
...
data = [
    ...
    ("GradientBoosting(T)",r2_gb_t,mse_gb_t,mae_gb_t,rmse_gb_t)
]
...
def predict_price(car_age, odometer,manufacturer,model,condition,cylinders,fuel,title_status,transmission,drive,ttype,paint_color,region):
   ...
   # numerica=["odometer","car_age"]
    ...
    x=int(predicted_price)
    y="Your Car Price is $"+str(x)
    return y
